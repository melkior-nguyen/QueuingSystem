...
function RoleAdd({ setCurrTopic }: any) {
  ...
  const [checkBoxA_1, setCheckBoxA_1] = useState<boolean>(false)
  const [checkBoxA_2, setCheckBoxA_2] = useState<boolean>(false)
  const [checkBoxA_3, setCheckBoxA_3] = useState<boolean>(false)
  const [checkBoxB_all, setCheckBoxB_all] = useState<boolean>(false)
  const [checkBoxB_1, setCheckBoxB_1] = useState<boolean>(false)
  const [checkBoxB_2, setCheckBoxB_2] = useState<boolean>(false)
  ...
  return (
    <div className="role_add-wrap">
      <h3 className='content_title'>Quản lý thiết bị</h3>
      <form className="role_add" onSubmit={(e) => handleRoleAddSubmit(e)}>
        <div className="role_add-content">
          <h3>Thông tin vai trò</h3>
          <div className="role_add-content-container">
            {/* left */}
            <div className="role_add-content-left">
              <span>Tên vai trò</span>
              <input type="text" onChange={(e) => setRoleName(e.target.value)} />
              <span>Mô tả</span>
              <textarea rows={6} placeholder='Chịu trách nhiệm...'
                onChange={(e) => setRoleDesc(e.target.value)}
              ></textarea>
            </div>
            {/* right */}
            <div className="role_add-content-right">
              <span>Phân quyền chức năng</span>
              <div className="role_add-group">
        ...
        <div className="role_add-btn">
          <button className="btn outline" type='button'
            onClick={() => setCurrTopic('system_role_list')}
          >Hủy</button>
          <button className="btn primary" type='submit'>Thêm vai trò</button>
        </div>
      </form>
    </div>
  )
